 ...
 */
class PrizeListController extends BaseController
{
    ...
    public function actionIndex()
    {
        ...
        $article_list = $query->select('articles.*,customs.name_zh as author_name,classes.name as class_name')
            ...
            ->where(['articles.school_id' => Yii::$app->session['manage_user']['school_id'], 'articles.article_type_id' => HintConst::$ARTICLE_PATH, 'articles.isdelete' => HintConst::$YesOrNo_NO]);
        if ($is_passed) {
...
//                //$command = $conn->createCommand("SELECT * FROM custom_prize_log as t1 LEFT JOIN zhuanpan_goods t2 WHERE t1.id= t2.goods_id");
//                //print_r($command);
//                //exit;
